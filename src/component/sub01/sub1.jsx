...
const Sub1 = () => {
    ...
    const isGenderSelected = (gender) => { //버튼이 클릭 됐는지 -> 클래스 이름 지정 -> 색상변경
        return selectedGender.includes(gender) ? styles.selectedGender : '';
    };

    const isAgeRangeSelected = (ageRange) => {//버튼이 클릭 됐는지 -> 클래스 이름 지정 -> 색상변경
        return selectedAgeRange.includes(ageRange) ? styles.selectedAgeRange : '';
    };

    const isConditionSelected = (condition) => {//버튼이 클릭 됐는지 -> 클래스 이름 지정 -> 색상변경
        return selectedCondition.includes(condition) ? styles.selectedCondition : '';
    };
    ...
    const handleButtonClick = (filterType, value) => { //카테고리 버튼 누르면 해당 버튼 추가 및 삭제 
        switch (filterType) {
            case 'gender':
            ...
            case 'ageRange':
                if (selectedAgeRange.includes(value)) {
                    setSelectedAgeRange(selectedAgeRange.filter(item => item !== value));
                } else {
            ...
            case 'condition':
                if (selectedCondition.includes(value)) {
                    setSelectedCondition(selectedCondition.filter(item => item !== value));
                } else {
    ...
    const handleApplyFilter = () => { //적용 버튼
        const filteredData = childrenData.filter(child => {
            return (
                (selectedGender.length === 0 || selectedGender.includes(child.gender)) &&
                (selectedAgeRange.length === 0 ||
                    selectedAgeRange.some(range => {
                        const [minAgeStr, maxAgeStr] = range.split('~').map(part => part.trim());
                ...
                (selectedCondition.length === 0 || selectedCondition.includes(child.condition))
            );
        });
    ...
    useEffect(() => {  // 페이지 로딩 시 모든 정보를 가져오는 부분
        axios.get("api/benefs?keyword=")
            .then((response) => {
                            ...
                            <div className={styles.filterDetailDiv}>
                                <div className={styles.detailTitle}>
                                    ...
                                    <div className={styles.sort}>
                                        <button className={`${styles.sex} ${isGenderSelected('남자')}`} onClick={() => handleButtonClick('gender', '남자')} >남자</button>
                                        <button className={`${styles.sex} ${isGenderSelected('여자')}`} onClick={() => handleButtonClick('gender', '여자')} >여자</button>
                                    </div>
                                </div>
                                <div className={styles.detailTitle}>
                                    <p className={styles.detailPLong}>연령대</p>
                                    <div className={styles.sort}>
                                        <button onClick={() => handleButtonClick('ageRange', '0~3')} className={`${styles.age} ${isAgeRangeSelected('0~3')}`}>0~3세</button>
                                        <button onClick={() => handleButtonClick('ageRange', '4~6')} className={`${styles.age} ${isAgeRangeSelected('4~6')}`}>4~6세</button>
                                        <button onClick={() => handleButtonClick('ageRange', '7~12')} className={`${styles.age} ${isAgeRangeSelected('7~12')}`}>7~12세</button>
                                        <button onClick={() => handleButtonClick('ageRange', '13~18')} className={`${styles.age} ${isAgeRangeSelected('0~3')}`}>13~18세</button>
                                    </div>
                                </div>
                                <div className={styles.detailTitle}>
                                    ...
                                    <div className={styles.sort}>
                                        <div className={styles.conditions}>
                                            <button onClick={() => handleButtonClick('condition', '주거지원')} className={`${styles.condition} ${isConditionSelected('주거지원')}`}>주거</button>
                                            <button onClick={() => handleButtonClick('condition', '학습지원')} className={`${styles.condition} ${isConditionSelected('학습지원')}`}>학습</button>
                                            <button onClick={() => handleButtonClick('condition', '재능지원')} className={`${styles.condition} ${isConditionSelected('재능지원')}`}>재능</button>
                                            <button onClick={() => handleButtonClick('condition', '생활지원')} className={`${styles.condition} ${isConditionSelected('생활지원')}`}>생활</button>
                                            <button onClick={() => handleButtonClick('condition', '의료지원')} className={`${styles.condition} ${isConditionSelected('의료지원')}`}>의료</button>
                                        </div>
                                        <div className={styles.conditions}>
                                            <button onClick={() => handleButtonClick('condition', '보육원')} className={`${styles.condition} ${isConditionSelected('보육원')}`}>보육원</button>
                                            <button onClick={() => handleButtonClick('condition', '조부모')} className={`${styles.condition} ${isConditionSelected('조부모')}`}>조부모</button>
                                            <button onClick={() => handleButtonClick('condition', '미혼모')} className={`${styles.condition} ${isConditionSelected('미혼모')}`}>미혼모</button>
                                            <button onClick={() => handleButtonClick('condition', '한부모')} className={`${styles.condition} ${isConditionSelected('한부모')}`}>한부모</button>
                                            <button onClick={() => handleButtonClick('condition', '다문화')} className={`${styles.condition} ${isConditionSelected('다문화')}`}>다문화</button>
                                        </div>
                                    </div>
                                </div>
